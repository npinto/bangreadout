...
def zmuv_rows_inplace_untested(
    ...
    cdef np.ndarray[DTYPE_t, ndim=1] _sum = np.zeros(rows, dtype=DTYPE)
    cdef np.ndarray[DTYPE_t, ndim=1] _ssq = np.zeros(rows, dtype=DTYPE)

    cdef Py_ssize_t i
    cdef Py_ssize_t j
    cdef DTYPE_t scaler = 1. / <DTYPE_t>cols
    cdef DTYPE_t val, _sum_i, _ssq_i, div
